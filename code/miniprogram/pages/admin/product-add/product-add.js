Page({
  ...
  fetchProductDetail: function(id) {
    ...
    wx.cloud.callFunction({
      ...
      success: (res) => {
        const data = res.result.recordsets && res.result.recordsets[0];
  ...
  handleSubmit: function() {
    ...
    if (this.data.isEdit) {
      ...
      sql = `INSERT INTO shangpin (mingcheng, xiangqing, fenlei, fenlei2, yuanjia, zhekou, shuliang, ztu, ltu1, ltu2, ltu3, beizhu, dianpu)
             VALUES ('${this.escape(p.mingcheng)}', '${this.escape(p.xiangqing)}', '${this.escape(p.fenlei)}', '${this.escape(p.fenlei2)}',
             ${this.num(p.yuanjia)}, ${this.num(p.zhekou)}, ${this.num(p.shuliang)},
             '${this.escape(p.ztu)}', '${this.escape(p.ltu1)}', '${this.escape(p.ltu2)}', '${this.escape(p.ltu3)}',
             '${this.escape(p.beizhu)}', '${this.escape(dianpu)}')`;
    }
